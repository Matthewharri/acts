...
void Acts::GeometryView3D::drawPortal(IVisualization3D& helper,
  ...
  //or one link(red)
  static ViewConfig good = ViewConfig({0, 255, 0});
  static ViewConfig bad = ViewConfig({255, 0, 0});
...
void Acts::GeometryView3D::drawDetectorVolume(IVisualization3D& helper, 
  ...
  //check if there are attached volumes, if yes draw, else recurse
  ...
  for (auto subvolume : subvolumes){
    if(subvolume->volumes().size() != 0){
      ...
      for(auto sub_portal : sub_portals){
        auto links = &(sub_portal->detectorVolumeUpdators());
        if(links->size() == 2){
